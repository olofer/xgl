...
% and quadrature routines for nodes on open interval (-1, +1).
%
...
% TODO: implement RGL Radau-Gauss-Lobatto on [-1, +1) or (-1, +1]
%    https://archive.lib.msu.edu/crcmath/math/math/r/r015.htm
...
Iq = quad(fx, -1, +1);  % Illustration of quadrature integration 
I0 = Ix(1) - Ix(-1);
...
xlabel('x');
ylabel('f(x)');
legend('CGL', 'LGL', 'OGL', 'f(x)', 'f(CGL)', 'f(LGL)', 'f(OGL)');
...
xlabel('x');
ylabel('df(x)/dx');
legend('CGL', 'LGL', 'OGL', 'df(x)/dx', 'df(CGL)/dx', 'df(LGL)/dx', 'df(OGL)/dx');
...
if nargin == 3 && k >= 1 && k < n
  ...
  dfxx = dfx(xx);
  nvec = (2:k:n)';
  err123 = zeros(length(nvec), 2);
  derr123 = zeros(length(nvec), 3);
  ierr123 = zeros(length(nvec), 3);
  for jj=1:length(nvec)
    ...
    derr123(jj, 3) = norm(dfxx - F3*D3*f3, 3);
  ...
  legend('CGL f', 'LGL f', 'OGL f', 'CGL df', 'LGL df', 'OGL df');
  xlabel(sprintf('#nodes (flipflag=%i)', flipflag));
  ylabel('log10(error)');
